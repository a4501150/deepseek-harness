...
`dsh-settings-file` 把所有 namespace 的用户设置保存在一个 YAML 或 JSON 文档中，默认是 harness home 下的 `settings.yaml`；配置的分拆文档——按路径与所辖 namespace 界定——可以把选定分节移到独立文件（例如一份 `model-settings.yaml` 承载模型相关 namespace），而服务看到的仍是一份合并文档：用户可以直接编辑文档——变更实时生效——也可以经服务写入，后者会安全合并并发编辑。YAML 写入保留每个未触碰节点上的注释、锚点与排版，未加载插件所拥有的分节也绝不会被丢弃。启动时非法文档直接报错；运行中失败的热重载保留最后可用分节并告警，而不是拖垮进程。
...
| `documents` | 无 | 分拆文档及其所辖 namespace；相对路径按主文档所在目录解析 |
...
每份文档都是 namespace 到用户分节的 YAML 或 JSON 映射。用户可以直接编辑：任何变更都会自动生效，删除文件则让该文件的分节回到默认值与 `base`。被路由的 namespace 恰好只有一个文件归属——它的拥有文档；主文档承载所有未被分拆文档认领的分节。写在拥有文档之外的被路由分节会在启动时迁入拥有文档（重复副本被丢弃）；启动之后，热重载幽灵只会在合成时被滤除并告警，而不会被写回。存在但非法的文档在启动时使插件加载失败——提供方绝不会静默忽略或覆盖它。运行中不可读或不可解析的编辑只告警并保留最后可用分节，因此手改出错不会拖垮进程。
...
- 两个分拆文档认领同一个 namespace、分拆文档指名主文档、或分拆路径重复，都会在加载时报错——有两个归属的分节只能靠巧合解析。
...
- **路由让每个 namespace 只有一个归属。** 写入路由到拥有文档，启动时把幽灵分节迁入其中，`compose` 再把各文档合并成 seam 的那份文档——存在错位位置的被路由分节永远不会进入 seam。
