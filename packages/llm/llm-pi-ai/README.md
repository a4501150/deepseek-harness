...
| `env` | none | Provider-scoped environment overlay this route's requests read instead of the process environment: `AWS_REGION`, `AWS_PROFILE`, `GOOGLE_CLOUD_PROJECT`, a proxy, and the like |
...
A model entry or `modelOverrides` value may declare a `pricing` block — per-million-token USD rates `input`, `output`, `cacheRead`, and `cacheWrite` — surfaced through `ctx.llm.resolveModelInfo()` and the browser model catalog so consumers can estimate spend. Only declared rates are deployment facts: the installed pi-ai catalog's own cost metadata is never read, an empty block declares nothing, and a model without pricing reports no cost rather than a guessed one.

### Run with reasoning and wire compatibility

`reasoningEfforts` declares a model's selectable thinking levels: each key is a level selectors offer, its value the spelling dispatch sends on the wire, so `max: ultra` renames a level for a gateway with its own vocabulary. Omitting the field keeps the installed catalog entry's capability; `false` declares a non-reasoning model. An entry's `defaultReasoningEffort` gives that one model the effort requests use when a caller names none, above the route's `reasoning` default, and it must name a level the model offers. `reasoningSummary` declares the summary mode (`auto`, `concise`, or `detailed`) that requests for that model send; unset and `off` both omit `reasoning.summary` — omission is the default, and the adapter strips the `auto` default pi-ai's request builder injects — so the provider runs no summarization pass and reports no visible thinking. A declaration on a model whose protocol never reads a `reasoning` parameter is refused. `compat` switches reshape the request for endpoints pi-ai cannot recognize — which role carries the system prompt, which field caps output, how a thinking level travels — configurable per route and per model. A model neither the entry nor the installed catalog sizes takes the route's `defaultContextWindow` and `defaultMaxTokens` fallbacks.
